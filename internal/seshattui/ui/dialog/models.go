...
func NewModels(com *common.Common, isOnboarding bool, preferredProviderID string) (*Models, error) {
	...
	m.keyMap.Back = key.NewBinding(key.WithKeys("left"), key.WithHelp("←", "back"))
...
func (m *Models) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		switch {
		case key.Matches(msg, m.keyMap.Back) && !m.isOnboarding:
			return ActionOpenDialog{DialogID: SettingsID}
		case key.Matches(msg, m.keyMap.Close):
			return ActionClose{}
		case key.Matches(msg, m.keyMap.Previous):
			m.list.Focus()
			m.list.SelectPrevCyclic()
		case key.Matches(msg, m.keyMap.Next):
			m.list.Focus()
			m.list.SelectNextCyclic()
...
func (m *Models) ShortHelp() []key.Binding {
	...
	h = append(h, m.keyMap.Back, m.keyMap.Close)
	return h
}
