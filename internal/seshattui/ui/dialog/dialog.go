...
// CloseSettingsFamilyDialogs closes every settings-family dialog that is
// mutually exclusive with the others. Call this before opening any dialog
// in the Settings/Commands/Models/Notifications/Reasoning group so that
// no stale dialog remains visible in the background.
func (d *Overlay) CloseSettingsFamilyDialogs() {
	d.CloseDialog(SettingsID)
	d.CloseDialog(CommandsID)
